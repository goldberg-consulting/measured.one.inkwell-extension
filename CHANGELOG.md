...
- **Hipster CV template.** New built-in `hipster-cv` template wrapping the `simplehipstercv` class (a mashup of hipstercv, friggeri, and twenty-seconds CV): full-width name banner, shaded sidebar (photo, about blocks, languages with skill dots, FontAwesome contact bubbles) driven entirely by YAML frontmatter, and a main column built from the markdown body. Raw LaTeX helpers `\cvevent{...}` (inside `cventries`) and `\cvyear{...}` (inside `cvyears`) render timeline entries with optional logos and year-indexed lists; `\cvevent`'s logo argument is now optional so entries without artwork stay aligned. Six color themes via `classoption`. Pandoc pipe tables are redirected from `longtable` (which cannot run inside `paracol`) to plain tabulars. Compiles with pdfLaTeX; new demo at `examples/demo-hipster-cv.md`, scaffold frontmatter stub, template regression check, and `fontawesome`, `raleway`, `paracol`, `smartdiagram`, `tikz-3dplot`, `pgf` added to `requirements-latex.txt` and the toolchain probe.
- **ETH Report typography and font controls.** The `eth-report` template now compiles with XeLaTeX + `fontspec` (manifest engine changed from `pdflatex`), so `mainfont` / `sansfont` / `monofont` select system OpenType fonts from frontmatter. The IVT class hardcodes 12 pt, its own A4 geometry, and one-half spacing; the wrapper now applies `fontsize:`, `geometry:`, and `linestretch:` after the class loads so documents can pick their own density (e.g. 10 pt protocols, 11 pt reports). Without a `mainfont` the template keeps the historical all-Inter sans look when Inter is installed, falling back to TeX Gyre Heros, then Latin Modern (`\IfFontExistsTF` guard, so CI runners without Inter still compile). The IVT class's `inputenc`/`fontenc`/`helvet` loads are guarded behind `\@ifpackageloaded{fontspec}` and remain intact for pdfLaTeX use of the class. Demo exercises `fontsize: 11pt`, `linestretch: 1.15`, and a `header-includes` command; new template regression checks pin the engine, load order, font guard, and citation fix.
- **Numeric citations by default.** Documents without a declared `csl:` now cite through a bundled numeric style: bracketed, comma-grouped in-text citations (`[@a; @b; @c]` renders as `[1,2,3]`) with a numbered reference list in first-citation order, applied identically in the PDF pipeline, the shell compile harness, and the live preview. Frontmatter `csl:` (searched in `.inkwell/csl/`, `csl/`, the project root, `references/`, and the document directory, with a warning when a declared `.csl` path resolves nowhere) or a `defaults.yaml` `csl:` entry overrides it. The bundled style deliberately avoids CSL `second-field-align`, which makes pandoc wrap entries in `\CSLLeftMargin`/`\CSLRightInline` parboxes that crash the tufte-latex wrappers with "Float(s) lost"; the tufte-book wrapper also gained the `\@biblabel{}` suppression every other wrapper already had, so numbered entries no longer carry a stray `[]`.
- **Section-level bibliographies.** `bibliography-scope: section` gives every top-level section (chapters in book templates; `section-bibs-level` moves the split deeper) its own reference list with per-section numbering, via a bundled Lua filter that runs citeproc per segment instead of `--citeproc` — raw `\part{...}` commands start a new segment so chapter references stay ahead of part pages. The document-level default is unchanged. The Tufte Book demo now ships with per-chapter reference lists as the working example; `scripts/compile-demo.sh` mirrors the flag logic so CI compiles what the extension compiles.
- **Python run-and-insert example.** New `examples/demo-python-report.md` walks the full loop — Run Code Blocks, then Compile — with a matplotlib figure, a generated CSV table, `::inkwell` scalar exports, bulk `vars.json` bindings, inline `{python}` expressions, and grouped numeric citations, so every number in its prose is computed. The compiled PDF and README screenshot are generated from actually-executed blocks.
- **Compile warnings for unresolved bindings.** A `{{key}}` placeholder that survives substitution (typo, stale payload, blocks never run) now surfaces as a compile warning with the source line — previously it shipped literally into the PDF with no signal. `vars.json` values must be flat scalars: objects and arrays are skipped with an explanatory log line instead of being stringified to `[object Object]` / comma-joined fragments.

### Fixed

- **Document `header-includes` were silently discarded whenever any `inkwell:` style key was set.** The generated style preamble was passed to Pandoc via `-H`, which fills the `header-includes` template variable and makes Pandoc ignore the document's own `header-includes` metadata — documents kept compiling while losing their font, spacing, table, and citation commands. The generated preamble is now merged into the template copy immediately before the `$for(header-includes)$` loop, so both blocks render and document commands override Inkwell styles (`-H` remains only as a fallback for third-party templates without a recognizable injection point, logged in the compile diagnostics). New `scripts/check-preamble-merge.mjs` (`npm run test:preamble`) asserts the merge against every shipped template.
- **Generated `code-font-size` styling disabled code-line wrapping.** The generated `Highlighting` redefinition restated only `fontsize`, dropping the templates' `breaklines`/`breakanywhere` options; long code lines overflowed the margin. The redefinition now preserves them.
- **ETH Report citations rendered as garbage labels like `[(1code )]`.** The IVT class loads natbib, and routing Pandoc citeproc through natbib's `\cite` mangled the structured `\bibitem[\citeproctext]` labels without any LaTeX error. `\citeproc` now prints the already-rendered citation text hyperlinked to the bibliography anchor.
- **A document's own `bibliography:` field was overridden by discovery.** Inkwell passes discovered `.bib` files as explicit `--bibliography` flags, and Pandoc lets command-line flags beat document metadata — so a document declaring a bibliography outside the scanned locations (project root, `references/`, `.inkwell/references/`) silently lost all its citations. Declared paths (scalar or list) now resolve against the document directory, then the project root, and are forwarded ahead of the discovered files; a declared file that does not exist produces a compile warning naming the path.
- **Preview ignored figure labels with additional attributes.** `![cap](img){#fig:name}` was registered for numbering and `@fig:` links only when the label was the entire attribute block; the common `{#fig:name width=80%}` form compiled correctly in the PDF but the preview showed the figure unnumbered with dead references. The preview now parses the full attribute block.

### Documentation

- guide.md: new ETH Report template reference (frontmatter, typography overrides, citation behavior — the template previously had no section), `header-includes` + `inkwell:` composition semantics, and bibliography resolution order.
- Agent guide (`.cursor/agents/inkwell-guide.md`): template inventory now includes ETH Report, Tufte Book VDQI, and Hipster CV (it stopped at the v0.2 set); the conversion advice to map Tufte margin notes to `::: {.aside}` divs — a syntax v0.3.3 removed because Pandoc silently unwraps it — is replaced with `\sidenote{}`/`\marginnote{}`; new ETH Report and Tufte Book sections.
