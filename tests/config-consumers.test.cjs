...
Module._load = function(request, ...rest) {
  ...
  if (request === './citation-pandoc') return { citationPandocEngine: { render: async () => undefined } };
  return originalLoad.call(this, request, ...rest);
};
...
for (const yaml of [
  ...
  test(`compiler and citation renderer share bibliography and CSL paths for ${JSON.stringify(yaml)}`, async t => {
    ...
    assert.deepEqual(resolved.bibliography, [declared]);
    ...
    assert.deepEqual([...citationResult.resolvedKeys].sort(), ['declared']);
    ...
    assert.deepEqual(bibliography, [...resolved.bibliography].reverse());
